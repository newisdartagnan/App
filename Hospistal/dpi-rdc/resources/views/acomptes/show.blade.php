...
<div class="max-w-5xl mx-auto px-4 py-6">
    ...
    <div class="grid grid-cols-3 gap-3 mb-5">
        <div class="bg-white rounded-xl shadow p-4 text-center">
            ...
            <p class="text-xs text-gray-500">Total avancé (CDF)</p>
        </div>
        <div class="bg-white rounded-xl shadow p-4 text-center">
        ...
        <div class="bg-white rounded-xl shadow p-4 text-center">
            ...
            <p class="text-xs text-gray-500">Encore disponible</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow overflow-hidden mb-5">
        ...
        <div class="px-4 py-3 border-b last:border-0">
            <div class="flex items-center justify-between flex-wrap gap-2">
                <div>
                    <span class="font-semibold text-gray-800">{{ number_format((float) $acompte->montant, 0, ',', ' ') }} {{ $acompte->devise }}</span>
            ...
            <p class="text-xs text-gray-600 mt-1">
                Imputé : <strong>{{ number_format((float) $acompte->montant_impute, 0, ',', ' ') }}</strong>
                · Remboursé : <strong>{{ number_format((float) $acompte->montant_rembourse, 0, ',', ' ') }}</strong>
                · Disponible : <strong class="{{ $acompte->resteDisponible() > 0 ? 'text-green-700' : '' }}">{{ number_format($acompte->resteDisponible(), 0, ',', ' ') }}</strong>
            </p>
            ...
            <ul class="mt-2 ml-4 text-xs text-gray-500 list-disc">
                ...
                <li>
                    {{ number_format((float) $imputation->montant, 0, ',', ' ') }} CDF sur
    ...
    <div class="bg-white rounded-xl shadow p-5">
        ...
        <p class="text-sm text-gray-600 mb-3">
            {{ number_format($disponible, 0, ',', ' ') }} CDF restent disponibles. Le remboursement
            impute d'abord les factures encore ouvertes, puis rend le solde au patient.
        </p>
